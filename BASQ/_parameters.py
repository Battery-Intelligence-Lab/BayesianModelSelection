import torch
import gpytorch
from ._quadrature import KernelQuadrature, ScaleKernelQuadrature
from ._sampler import PriorSampler, UncertaintySampler, LogUncertaintySampler
from ._wsabi import WsabiGP
from ._vbq import VanillaGP
from ._scale_mmlt_wsabi import ScaleMmltWsabiGP
...
def parameter_set():
    # BQ Modelling
    params = {
    "bq_model": "mmlt",              # select a BQ model from ["mmlt", "wsabi", "vbq"], vbq stands for Vanilla BQ
    ...
    "ratio": 1,                      # mixing ratio of prior and uncertainty sampling, 0 < r < 1
    "n_gaussians": 50,               # number of Gaussians approximating the GP-modelled acquisition function
    "threshold": 0,                  # threshold to cut off the insignificant Gaussians
...
class Parameters:
    def __init__(self, Xobs, Yobs, prior, true_likelihood, device):
        ...
        self.load_setting(params, Xobs, Yobs, prior, true_likelihood, device)

    def load_setting(self, params, Xobs, Yobs, prior, true_likelihood, device):
        """
        Args:
           - params; dict, the dictionary of the parameters
           - Xobs; torch.tensor, X samples, X belongs to prior measure.
           - Yobs; torch.tensor, Y observations, Y = true_likelihood(X).
           - prior; torch.distributions, prior distribution.
           - true_likelihood; function of y = function(x), true likelihood to be estimated.
        ...
        self.bq_model = params["bq_model"]
        self.sampler_type = params["sampler_type"]
        self.check_compatibility(self.bq_model, self.sampler_type, params["kernel_type"])
        gp_kernel = self.set_kernel(params["kernel_type"])
        self.set_model(
            self.bq_model,
            Xobs,
            Yobs,
        ...
        self.set_quadrature(
            params["nys_ratio"],
            int(self.n_rec * params["nys_ratio"]),
            int(self.n_rec * params["quad_ratio"]),
            self.bq_model,
        )
        self.set_posterior()
        self.verbose(
    ...
    def set_sampler(self, sampler_type, sampling_method, bq_model, prior, n_rec, nys_ratio, ratio, n_gaussians, threshold):
        """
        Args:
           - sampler_type; string, ["uncertainty", "prior"]
           - sampling_method; string, ["exact", "approx"]
           - bq_model: string, ["scale_mmlt", "mmlt", "wsabi", "vbq"]
           - prior: torch.distributions, prior distribution.
        ...
        if sampler_type == "uncertainty":
            if bq_model == "wsabi":
                ...
                self.kernel_quadrature = self.gp.kernel
            elif bq_model == "mmlt":
                self.sampler = LogUncertaintySampler(
                    prior,
                    self.gp,
                    n_rec,
                    nys_ratio,
                ...
                self.kernel = self.gp.gspace_kernel
                self.kernel_quadrature = self.gp.fspace_kernel
            else:
                raise Exception("The given bq_model is not compatible with the uncertainty sampler.")
        elif sampler_type == "prior":
            self.sampler = self.prior_sampler
    ...
    def set_model(self, bq_model, Xobs, Yobs, gp_kernel, wsabi_type, alpha_factor, lik, training_iter, thresh, lr, rng, train_lik, optimiser):
        ...
        if bq_model == "wsabi":
            ...
            self.predict_mean = self.gp.fspace_mean_predict
            self.predict = self.gp.fspace_predict
            self.retrain = self.gp.retrain_gp_with_scaling
            self.kernel = self.gp.fspace_kernel
            self.update = self.gp.update_mmlt_gp_with_scaling
    ...
    def set_quadrature(self, nys_ratio, n_nys, n_quad, bq_model):
        """
        Args:
    ...
    def check_compatibility(self, bq_model, sampler_type, kernel_type):
        ...
        if bq_model == "wsabi":
            ...
            if not sampler_type == "prior" and not bq_model == "mmlt":
                raise AssertionError("Uncertainty sampling requires WSABI-L modelling with RBF kernel.")

        if sampler_type == "uncertainty":
            if not kernel_type == "RBF" or bq_model == "vbq":
                raise AssertionError("Uncertainty sampling requires RBF kernel.")
