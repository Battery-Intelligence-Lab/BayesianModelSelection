...
class KernelQuadrature:
    ...
    def quadrature(self):
        ...
        self.EZy = (w @ self.mean_predict(X)).item()
        VarZy = (w @ self.kernel(X, X) @ w).item()
        print("E[Z|y]: " + str(self.EZy) + "  Var[Z|y]: " + str(VarZy))
        return self.EZy, VarZy
...
class ScaleKernelQuadrature(KernelQuadrature):
    def __init__(self, n_rec, n_nys, n_quad, batch_size, sampler, gp, device):
        """
        Args:
           - n_rec: int, subsampling size for kernel recombination
           - nys_ratio: float, subsubsampling ratio for Nystrom.
