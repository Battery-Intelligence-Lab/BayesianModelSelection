...
class GaussianCalc:
    ...
    def calc_expGMM(self, a):
        Npdfs = MultivariateNormal(
            a,
            self.W / 2,
        ).log_prob(self.X_ij_plus_half).exp()
        return torch.exp(-1 * Npdfs @ self.Wij_flat)

    def calc_Taylor(self, mus):
        return torch.stack([self.calc_expGMM(mu) for mu in mus])
