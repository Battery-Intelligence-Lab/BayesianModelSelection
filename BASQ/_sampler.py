...
from ._acquisition_function import SquareRootAcquisitionFunction, LogRootAcquisitionFunction
...
class UncertaintySampler(SquareRootAcquisitionFunction):
    ...
    def calc_weights(self, pts_rec):
        """
        weights w_IS = f(x) / g(x)
        g(x) = (1-r) f(x) + r var(x)π(x),
        f(x) = |m(x)| π(x)
        warped GP(m(x), var(x,x))
...
class LogUncertaintySampler(LogRootAcquisitionFunction):
    def __init__(
        self,
        prior,
        gp,
        ...
        Args:
           ...
           - threshold: float, threshold to cut off the insignificant Gaussians
        """
        super().__init__(prior, gp.model, device, n_gaussians=n_gaussians, threshold=threshold)
        self.gp = gp
        self.ratio = ratio
        self.nys_ratio = nys_ratio
        self.ratio_super = ratio_super
    ...
    def SIR_from_mean(self, n_super, n):
        ...
        """
        X_pi = self.sampling_mean(n_super)
        mu_g = self.gp.gspace_mean_predict(X_pi)
        mu_log = mu_g.abs().log()
        prior_log = self.utils.safe_mvn_prob(self.prior.loc, self.prior.covariance_matrix, X_pi).log()
        sampler_log = torch.nan_to_num(self.joint_pdf_mean(X_pi)).log()
    ...
    def SIR_from_AF(self, n_super, n):
        ...
        """
        X_A = self.sampling(n_super)
        _, var_A = self.gp.gspace_predict(X_A)
    ...
    def calc_weights(self, pts_rec):
        """
        gspace GP(mu_g(x), var_g(x,x))
        weights w_IS = f(x) / g(x)
        g(x) = (1-r) f(x) + r var_g(x)π(x),
        f(x) = |mu_g(x)| π(x)
        ...
        Returns:
           - w_IS: torch.tensor, weights for importance sampling if pts_rec is not sampled from the prior
        """
        mu_g_rec, var_g_rec = self.gp.gspace_predict(pts_rec)
        f_rec = torch.exp(torch.abs(mu_g_rec).log() + self.prior.log_prob(pts_rec))
        if self.ratio < 1:
            g_rec = torch.exp(
                torch.log(self.ratio * var_g_rec + (1 - self.ratio) * torch.abs(mu_g_rec))
                + self.prior.log_prob(pts_rec)
            )
        else:
            g_rec = torch.exp(
                torch.tensor(self.ratio).log() + var_g_rec.log() + self.prior.log_prob(pts_rec)
            )
        w_IC = f_rec / g_rec
        w_IC = w_IC / w_IC.sum()
        return w_IC
    ...
    def exact(self, n):
        """
        Proposal distribution g(x) = (1-r) f(x) + r var_g(x)π(x),
                              f(x) = |mu_g(x)| π(x),
        where r is self.ratio, m(x) and C(x) are the mean and varinace of square-root kernel.
        weights w_IS = f(x) / g(x)
        samples for Nystrom should be sampled from f(x), thus we adopt SIR.
